...
| ---------------------- | -------------- | --------------------------------------------------- |
| **Frontend (Astro)**   | ✅ Complete    | Integrated via API Gateway (v1)                     |
| **Sanity Studio**      | ✅ Complete    | Schemas: post, category, tag.                       |
| **CMS Service**        | ✅ Complete    | NestJS, Strict Typing, Mappers, Standardized Errors |
| **API Gateway**        | ✅ Complete    | NestJS, Transparent Proxy, api/v1, Rate Limiting    |
| **Kafka Infra**        | ⚠️ Pending     | Docker layer not yet configured                     |
| **AI Service**         | 🕒 Shell       | Proxy ready, logic pending                          |
| **Blockchain Service** | 🕒 Shell       | Proxy ready, logic pending                          |
...
3. **Always update `DEVELOPMENT-STATUS.md`** at the end of a phase.
4. **Mandatory Linter**: Run `npm run lint` before finishing any feature/fix.
5. **Keep secrets out of code**: No Keys, IDs or tokens in docs or code; use `.env`.
6. **Follow Clean Architecture**: controllers → services → repositories → external clients
7. **Use semantic commits**:
   - `feat:` new feature
   - `fix:` bug fix
   - `docs:` documentation
...
| ----- | ------------------------------------ | ---------------- | ------------------------------- |
| 🌱 1  | Foundation (CMS + Gateway + Astro)   | **80% Complete** | Production architecture running |
| ⚙️ 2  | Business Logic (Payments + Auth)     | Not Started      | OAuth2 + Payment flow           |
| 🧠 3  | AI Content                           | **Initiated**    | Shells & Proxies ready          |
| 🔗 4  | Blockchain                           | **Initiated**    | Shells & Proxies ready          |
...
- [ ] Replace direct Sanity calls with API Gateway calls:
  ...
  const API_URL =
    import.meta.env.PUBLIC_API_GATEWAY_URL || "http://localhost:3000/api";
...
> Coreflow is a _developer showcase ecosystem_ —  
